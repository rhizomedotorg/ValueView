( function( $ ) {
	...
	var PLUGIN_NAME = 'inputautoexpand';
...
$.fn.inputautoexpand = function( options ) {
	this.filter( 'input:text, textarea' ).each( function() {
		...
		if( instance ) {
			...
			$.data( this, PLUGIN_NAME, new $.InputAutoExpander( this, options ) );
		}
	} );
 ...
 * @see jQuery.fn.inputautoexpand
 * @class jQuery.InputAutoExpander
 ...
 * @param {Object} [options] See jQuery.fn.inputautoexpand for default options.
 */
$.InputAutoExpander = function( element, options ) {
	var self = this;
	...
	this._$input.on( 'eachchange.' + PLUGIN_NAME, function( event, oldValue ) {
		if( self._options.suppressNewLine ) {
			self._$input.val( self._$input.val().replace( /\r?\n/g, '' ) );
		}
		self.expand();
	} );
	...
	this._$input.css( 'resize', 'none' );

	$( window )
	.off( '.' + PLUGIN_NAME )
	.on( 'resize.' + PLUGIN_NAME, function( event ) {
...
$.extend( $.InputAutoExpander.prototype, {
	/**
	...
	expand: function( force ) {
		var newVal = this._$input.val();

		if( newVal === this._previousVal && !force ) {
			return;
		}
	...
	_expandWidth: function() {
		copySpaceAffectingStyles( this._$input, $rulerX );
		...
		var valWidth = this._getWidthFor( this._$input.val() ),
	...
	_expandHeight: function() {
		copySpaceAffectingStyles( this._$input, $rulerY );

		var newHeight = this._getHeightFor( this._$input.val() ),
			input = this._$input.get( 0 ),
	...
	_getMinWidth: function() {
		...
		if( !this._$input.attr( 'placeholder' ) ) {
			return 0;
		}
		// Don't need comfort zone in this case, just some sane space:
		return Math.ceil( this._getWidthFor( this._$input.attr( 'placeholder' ) + ' ' ) );
	},
	...
	_getHeightFor: function( text ) {
		...
		if( !this._options.expandWidth || this._$input.width() > this._options.maxWidth ) {
			width = Math.ceil( this._$input.width() ) - 1;
		}
		...
		var border = parseInt( this._$input.css( 'borderTopWidth' ), 10 )
			+ parseInt( this._$input.css( 'borderBottomWidth' ), 10 );
	...
	_getComfortZone: function() {
		return this._options.comfortZone || Math.ceil( this._getWidthFor( ' ' ) * 2 );
	},
	 ...
	 */
	destroy: function() {
		$.removeData( this._$input.get( 0 ), 'inputautoexpand' );
